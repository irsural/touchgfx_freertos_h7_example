...
class codecvt_cp866 : public std::codecvt<uint16_t, char, mbstate_t>
{
  public:
...
protected:
    ...
    const
  {
    while(from != from_end)
    {
      ...
      if(ufrom <= 0x7F)
        ...
        *to = static_cast<uint16_t>(ufrom + 0x390);
      else if(0xE0 <= ufrom && ufrom <= 0xEF)
        *to = static_cast<uint16_t>(ufrom + 0x360);
      else
      {
        std::map<unsigned char, uint16_t>::const_iterator s;
  ...
  virtual result do_out(mbstate_t&,
                        const uint16_t* from,
                        const uint16_t* from_end,
                        const uint16_t* &from_next,
                        char* to,
                        char* to_end,
                        char* &to_next)
    const
  {
    while(from != from_end)
    {
      ...
      {
        std::map<uint16_t, unsigned char>::const_iterator s;
  ...
  /*
  virtual int do_length(mbstate_t& s, const char* from, const char* from_end, size_t max) const
  {
    return codecvt<wchar_t, char, mbstate_t>::do_length(s, from, from_end, max);
  }

  virtual int do_max_length() const throw()
  {
    return codecvt<wchar_t, char, mbstate_t>::do_max_length();
  }
*/
private:
  std::map<unsigned char, uint16_t> in_tab;
  std::map<uint16_t, unsigned char> out_tab;
};
